...
from (
    ...
    from dw_evaluation.community_month_report_base_info t1
             inner join (select distinct month_short_desc
                         ...
                         where month_short_desc >= substring(cast(add_months(current_timestamp(), -12) as string), 1, 7)
    ...
    where substring(deal_date, 1, 7) >= substring(cast(add_months(current_timestamp(), -12) as string), 1, 7)
