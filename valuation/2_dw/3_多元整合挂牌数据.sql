...
insert overwrite table dw_evaluation.house_valuation_rack_community_area_interval
    select
        ...
        sum(case when t1.house_avg_price is not null then t1.house_avg_price else 0 end )/count(case when t1.house_avg_price is not null then 1 else 0 end) as community_avg_price,
